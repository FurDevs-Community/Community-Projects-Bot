...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (args.length < 2) return message.reply({ content: "You need to tell me who will be getting the project and the reason. (missing args: `newOwner (user)`, `reason`" })
    ...
    if (actualCheck) {
      ...
      if (inserver) {
        ...
        const usurekigu = new EmbedBuilder()
          ...
          .setDescription(`Do you wish to transfer this project's ownership to <@${repuser}>?`)
          .setTimestamp()
        ...
        const Log = new EmbedBuilder()
          ...
          .addFields([
            { name: "Channel involved:", value: `<#${message.channel.id}> (ID: ${message.channel.id})` },
            { name: "Action Type:", value: "**Forced Ownership Transfer**" },
            { name: "Old Owner:", value: `<@${check.projectLeader}> (UID: ${check.projectLeader})`, inline: true },
          ...
          collector.on('collect', async i => {
            if (i) {
              ...
              if (i.customId == 'bignono') {
                ...
                await message.channel.permissionOverwrites.edit(`${check.projectLeader}`, {
                  ViewChannel: true,
                  SendMessages: true,
                  ManageWebhooks: true
                })
